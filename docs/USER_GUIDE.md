...
*   **Add Files** / **Add Folder**: load individual images or every image in a folder into this session. Pick a folder that holds only *other* folders and NegPy reveals it in the Library section instead of reporting that it found nothing. Dropping a folder on the window does the same — most of the time that is the faster path anyway.
