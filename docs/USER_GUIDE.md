...
*   **Positive Source** (default off): on Transparency with Normalize off, NegPy otherwise reads the source as literal linear data and renders it the way a raw converter opens an underexposed sensor capture — a fixed +0.7 EV lift plus a filmic highlight roll-off, both correct for a raw scanner or camera capture and both too much for a source that is already a finished, correctly-exposed image. Turn this on when the source is already a finished positive — a scanned print, an export from other software — and NegPy decodes its own embedded profile instead of assuming linear data (an untagged file gets the same sRGB assumption an untagged 8-bit file already gets, since a positive source has resolved the usual ambiguity a bare untagged 16-bit file carries), and skips that fixed lift and roll-off so the Print sliders shape the capture directly rather than a re-rendered version of it. Live only on that as-captured transfer; greyed out everywhere else, including Transparency with Normalize on, which already decodes on its own profile.
