...
def test_reveal_section_switches_to_roll_for_the_geometry_and_flat_field_cards():
    """Auto Crop, Lens Correction and Flat Field moved to the Roll tab, so the ⓘ and
    the search must stop looking for them among Frame's sub-tabs."""
    for attr in ("autocrop_section", "lens_section", "flatfield_section"):
        panel = _group_panel_stub()
        panel._section_tab_index = {}
